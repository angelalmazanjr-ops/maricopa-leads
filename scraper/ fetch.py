...
def build_url(code, dfrom, dto, start=0):
    b = dfrom.replace("/", "%2F")
    e = dto.replace("/", "%2F")
    return f"{RECORDER_SEARCH}?rec={start}&suf=&nm=&bdt={b}&edt={e}&cde={code}&max=100&res=True&doc1={code}"
    ...
    for row in table.find_all("tr")[1:]:
        ...
        try:
            ...
            if not num:
                continue
            records.append({
                "doc_num": num, "doc_type": raw or code, "cat": code, "cat_label": label,
                "filed": _nd(date), "owner": "", "grantee": "", "legal": "", "amount": None,
                "clerk_url": url, "prop_address": "", "prop_city": "", "prop_state": "AZ",
                "prop_zip": "", "mail_address": "", "mail_city": "", "mail_state": "AZ", "mail_zip": "",
            })
        except Exception as e:
            log.debug(f"Row: {e}")
    return records
    ...
    try:
        ...
        for row in soup.find_all("tr"):
            ...
            if len(cells) >= 2:
                ...
                if "grantor" in lbl and not r["owner"]:
                    r["owner"] = val
                elif "grantee" in lbl and not r["grantee"]:
                    r["grantee"] = val
                elif "legal" in lbl and not r["legal"]:
                    r["legal"] = val
                elif "consideration" in lbl and not r["amount"]:
                    r["amount"] = _pa(val)
    except Exception as e:
        log.debug(f"Detail: {e}")
    return r
...
def score(rec, all_r):
    ...
    try:
        if (datetime.now().date() - datetime.strptime(filed, "%Y-%m-%d").date()).days <= 7:
